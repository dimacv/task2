...
Дополнительно: соблюдение PEP8, использование для визуализации библиотек bokeh или seaborn.
...
### Подход к решению
 ...
 1. Проверка ряда на стационарность. Это можно сделать двумя способами: 
 ...
 2. Построение графиков автокорреляции и частичной автокорреляции для определения параметров p, q модели ARIMA и SARIMAX.
 3. Нахождения порядка интегрированности временного ряда, для этого был получен ряд первых разностей. Затем была проведена проверка на стационарность полученного ряда. 
 4. Подбор параметров p, q проводился перебором с помощью кросс-валидации. Сравнение моделей производилось с помощью r2_score. 
 5. Обучение модели SARIMAX с параметрами p,d,q полученными из графиков функции автокореляции и частичной автокореляции. Обучение второй модели SARIMAX с параметрами, полученными с помощью перебора. Сравнение полученных моделей с помощью информационного критерия Акаике.
 6. Получение предсказания выбранной модели SARIMAX. Подсчет r2_score.
